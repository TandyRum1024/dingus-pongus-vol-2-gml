...
void main()
{
    float weights[2] = float[2](0.382925, 0.382925); 
    // gl_FragColor = v_vColour * texture2D( gm_BaseTexture, v_vTexcoord );
    
    vec4 bloomTint = vec4(1.0);
    float qualityCoeff = 0.1 / quality * uBloomness;
    
    vec2 noise = qualityCoeff * (vec2(rand(v_vTexcoord), rand(v_vTexcoord.yx)) - vec2(0.5)) * 0.01;
    
    vec2 o1 = kernel.z * noise + v_vTexcoord + vec2(uPixelSize.x * 2.0, 0.0);
    vec2 o2 = kernel.y * noise + v_vTexcoord + vec2(uPixelSize.x, 0.0);
    vec2 o3 = kernel.x * noise + v_vTexcoord + vec2(0.0, 0.0);
    vec2 o4 = kernel.y * noise + v_vTexcoord + vec2(-uPixelSize.x, 0.0);
    vec2 o5 = kernel.z * noise + v_vTexcoord + vec2(-(uPixelSize.x * 2.0), 0.0);
